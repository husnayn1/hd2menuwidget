...
import { EntertainmentKonva } from "../entertainment";
...
function CustomizeClockModal({ widget, onClose, onSave, onBack }) {
  ...
  const [countdownTitle, setCountdownTitle] = useState("Countdown App");
  const [countdownSubtitle, setCountdownSubtitle] = useState("Be Ready");
  ...
  const isClock = widget?.name?.toLowerCase().includes("clock") ||
                  widget?.originalName?.toLowerCase().includes("clock") ||
                  ...
                  // Check for clock-specific properties that indicate this is a clock widget
                  widget?.dialType ||
                  widget?.dialBg ||
                  widget?.handsColor;
  ...
  const isCountdown = widget?.type === "countdown-app" || widget?.name?.toLowerCase().includes("countdown");
  ...
  useEffect(() => {
    if (widget) {
      ...
      setCountdownTitle(widget.countdownTitle || widget.title || "Countdown App");
      setCountdownSubtitle(widget.countdownSubtitle || widget.subtitle || "Be Ready");
    ...
    <>
      <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(0,0,0,0.3)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
        <div style={{ background: '#f5f5f5', width: 800, height: 600, borderRadius: 8, position: 'relative', display: 'flex', boxShadow: '0 4px 24px rgba(0,0,0,0.12)', overflow: 'hidden' }}>
            ...
            <div style={{ background: '#fff', border: '1px solid #ddd', borderRadius: widget?.type === 'analog-square' ? 0 : 8, width: 200, height: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: 15 }}>
              {(() => {
                ...
                if (clockType === 'digital-simple') {
                  // Show static image for digital simple clock in left panel preview
                  return <img src={widget?.image || "https://via.placeholder.com/180x180?text=Digital+Clock+Preview"} alt="Preview" style={{ width: 180, height: 180, objectFit: 'contain' }} />;
                } else if (clockType === 'bar-modern') {
                  // Show static image for bar modern clock in left panel preview
                  return <img src={widget?.image || "https://via.placeholder.com/180x180?text=Bar+Clock+Preview"} alt="Preview" style={{ width: 180, height: 180, objectFit: 'contain' }} />;
                } else if (clockType === 'glow-clock') {
                  // Show static image for glow clock in left panel preview
                  return <img src={widget?.image || "https://via.placeholder.com/180x180?text=Glow+Clock+Preview"} alt="Preview" style={{ width: 180, height: 180, objectFit: 'contain' }} />;
                } else if (clockType === 'countdown-app') {
                  // Show static image for countdown app in left panel preview
                  return <img src={widget?.image || "https://via.placeholder.com/180x180?text=Countdown+Preview"} alt="Preview" style={{ width: 180, height: 180, objectFit: 'contain' }} />;
                } else if (clockType === 'analog-round' || clockType === 'analog-square') {
                  // Show static image for analog clocks in left panel preview
                  return <img src={widget?.image || "https://via.placeholder.com/180x180?text=Analog+Clock+Preview"} alt="Preview" style={{ width: 180, height: 180, objectFit: 'contain' }} />;
                } else {
          ...
          <div className="widget-form" style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, background: '#fff' }}>
            {/* Header */}
              ...
              {(activeTab === 'settings' || isClock) && (
                <>
                    ...
                    <>
                      <div style={{ marginBottom: 12 }}>
                        ...
                        <select 
                          value={dialType} 
                          onChange={e => {
                            console.log('Dial type changed from', dialType, 'to', e.target.value);
                            setDialType(e.target.value);
                          }} 
                          style={{ width: '100%', padding: 6, borderRadius: 4, border: '1px solid #ccc', fontSize: 14 }}
                        >
                  ...
                  {isCountdown && (
                    <>
                      <div style={{ marginBottom: 12 }}>
                        <div style={{ fontWeight: 'bold', marginBottom: 4, fontSize: 14 }}>Target Date & Time</div>
                        <input
                          type="datetime-local"
                          value={eventDateTime}
                          onChange={e => setEventDateTime(e.target.value)}
                          style={{ width: '100%', padding: 6, borderRadius: 4, border: '1px solid #ccc', fontSize: 14 }}
                        />
                        <div style={{ fontSize: 11, color: '#666', marginTop: 2 }}>Set the date and time to count down to</div>
                      </div>
                      ...
                      <div style={{ marginBottom: 12 }}>
                        ...
                        />
                        <div style={{ fontSize: 11, color: '#666', marginTop: 2 }}>Message to show when countdown reaches zero</div>
                      </div>
                      ...
                      <div style={{ marginBottom: 12 }}>
                        ...
                        <input
                          ...
                          style={{ width: '100%', padding: 6, borderRadius: 4, border: '1px solid #ccc', fontSize: 14 }}
                          placeholder="Countdown App"
                        />
                        <div style={{ fontSize: 11, color: '#666', marginTop: 2 }}>Set a text that will appear at top of the app.</div>
                      </div>
                      ...
                      <div style={{ marginBottom: 12 }}>
                        ...
                        <input
                          ...
                          placeholder="Be Ready"
                        />
                        <div style={{ fontSize: 11, color: '#666', marginTop: 2 }}>Set a text that will appear above the title.</div>
                      </div>
                      ...
                      <div style={{ marginBottom: 12 }}>
                        <div style={{ fontWeight: 'bold', marginBottom: 4, fontSize: 14 }}>Text Color <span style={{ color: '#888', fontWeight: 'normal' }}>(optional)</span></div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                          <input 
                            type="color" 
                            value={fontColor} 
      ...
      {isClock && showFullscreenPreview && (
        <div style={{ 
          ...
          <Clock
            key={`fullscreen-${dialType}-${dialBg}-${handsColor}-${bgColor}`}
            type={(() => {
              ...
              console.log('Fullscreen preview - clockType:', clockType, 'dialType:', dialType);
              return clockType;
            })()}
            size={Math.min(window.innerWidth * 0.4, window.innerHeight * 0.4, 400)}
            dialType={dialType}
            dialBg={dialBg}
