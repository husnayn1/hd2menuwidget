...
function FinanceCustomizeModal({ widget, onClose, onSave, onBack }) {
  ...
  const handlePreview = () => {
    if (!baseCurrency || !exchangeCurrency) {
      alert('Please select both base and exchange currencies before previewing.');
      return;
    }
            ...
            <div style={{ flex: 1, padding: '20px', overflowY: 'auto' }}>
              ...
              <div style={{ padding: '15px 20px', borderTop: '1px solid #eee', background: '#fff', display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
                <button 
                  ...
                  style={{ 
                    background: (!appName.trim() || !baseCurrency || !exchangeCurrency) ? '#ccc' : '#1976d2', 
                    color: '#fff', 
                    border: 'none', 
                    borderRadius: 4, 
                    padding: '8px 20px', 
                    fontWeight: 'bold', 
                    fontSize: 14, 
                    cursor: (!appName.trim() || !baseCurrency || !exchangeCurrency) ? 'not-allowed' : 'pointer' 
                  }}
                  disabled={!appName.trim() || !baseCurrency || !exchangeCurrency}
                >
                  Preview
                </button>
                <button 
