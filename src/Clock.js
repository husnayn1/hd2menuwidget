...
function drawNumbers(dialType, handsColor, type, size) {
  ...
  if (type === "analog-square") {
    console.log('Square clock - dialType:', dialType);
    // Only 12, 3, 6, 9 - but respect dialType like round clock
    if (dialType !== "none") {
      const center = size / 2;
      ...
      positions.forEach((pos, i) => {
        let label = pos.n.toString();
        if (dialType === "roman") {
          const roman = ["XII", "III", "VI", "IX"];
          label = roman[i];
          console.log(`Square clock using Roman: ${pos.n} -> ${label}`);
        }
        // For dialType === "arabic" or default, use label = pos.n.toString() (already set above)
        numbers.push(
          <Text
            key={i}
            x={pos.x - size * 0.1}
...
function AnalogClock({ dialType = "arabic", dialBg = "#fff", handsColor = "#000", bgColor = "#fff", type, size = 220 }) {
  console.log('AnalogClock received dialType:', dialType, 'type:', type);
  const [time, setTime] = useState(new Date());
  useEffect(() => {
    const interval = setInterval(() => setTime(new Date()), 1000);
    return () => clearInterval(interval);
  ...
  return (
    <Stage width={size} height={size} style={{ background: bgColor, borderRadius: type === "analog-square" ? 0 : '50%' }}>
      <Layer>
        ...
        {/* Ticks */}
        {drawTicks(type, handsColor, size)}
        {/* Numbers */}
...
function CountdownClock({ size = 220, targetDate, message, title, subtitle, textFont, fontColor, bgColor }) {
  ...
  const [target, setTarget] = useState(() => 
    targetDate ? new Date(targetDate) : new Date(Date.now() + 2 * 24 * 60 * 60 * 1000)
  );
  const [now, setNow] = useState(new Date());
  
  // Update target when targetDate prop changes
  useEffect(() => {
    const newTarget = targetDate ? new Date(targetDate) : new Date(Date.now() + 2 * 24 * 60 * 60 * 1000);
    setTarget(newTarget);
  }, [targetDate]);
  ...
  // Debug logging
  console.log('CountdownClock Debug:', {
    targetDate,
      ...
      <div style={{ flex: 1, padding: size * 0.09, display: 'flex', flexDirection: 'column', justifyContent: 'flex-start', minWidth: 0 }}>
        ...
        <div style={{ display: 'flex', gap: size * 0.06, alignItems: 'flex-end', marginBottom: size * 0.04 }}>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: size * 0.14, fontWeight: 700, letterSpacing: 2, color: mainColor }}>{String(days).padStart(2, '0')}</div>
            <div style={{ fontSize: size * 0.045, color: '#e74c3c', fontWeight: 600, marginTop: 2, letterSpacing: 1 }}>DAYS</div>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: size * 0.14, fontWeight: 700, letterSpacing: 2, color: mainColor }}>{String(hours).padStart(2, '0')}</div>
            <div style={{ fontSize: size * 0.045, color: '#e74c3c', fontWeight: 600, marginTop: 2, letterSpacing: 1 }}>HOURS</div>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: size * 0.14, fontWeight: 700, letterSpacing: 2, color: mainColor }}>{String(minutes).padStart(2, '0')}</div>
            <div style={{ fontSize: size * 0.045, color: '#e74c3c', fontWeight: 600, marginTop: 2, letterSpacing: 1 }}>MINUTES</div>
          </div>
          <div style={{ textAlign: 'center' }}>
            <div style={{ fontSize: size * 0.14, fontWeight: 700, letterSpacing: 2, color: mainColor }}>{String(seconds).padStart(2, '0')}</div>
...
function Clock({ dialType = "arabic", dialBg = "#fff", handsColor = "#000", bgColor = "#fff", type, size = 220, targetDate, message, title, subtitle, textFont, fontColor, ringColor, bgCircleColor, timeFormat, ampm }) {
  ...
  // fallback
  return <div style={{ width: size, height: size, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#888' }}>No preview</div>;
}
